    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F8FAFC', display: 'flex', justifyContent: 'center' }}>
      ...
      <div style={{ flex: 1, maxWidth: '800px', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <main style={{ padding: '20px 16px 80px 16px', flex: 1 }}>
          ...
          {/* DEDICATED FULL PROFILE VIEW IF A PROVIDER IS SELECTED */}
          {selectedProvider ? (
            <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {selectedProvider.role === 'DEUTSCH_INSTITUT' ? (
                <DeutschInstitutProfile
                  ...
                  shortBio="Centre agréé spécialisé pour l'apprentissage de la langue allemande et la préparation au Goethe-Zertifikat A1-C1."
                  fullDescription={`L'Institut Goethe Partner Cameroon forme les étudiants aux exigences linguistiques et culturelles allemandes.\n\nServices:\n- Enseignants certifiés\n- Salles de cours climatisées\n- Examens blancs hebdomadaires.`}
                  yearFounded={2018}
                  followerCount={380}
                  verified={selectedProvider.verified}
                  onContactClick={() => {
                    setToastMessage(`Redirection vers la messagerie de ${selectedProvider.name}`);
                    setSelectedProvider(null);
                ...
                <IndividualProviderProfile
                  ...
                  roleName={
                    selectedProvider.role === 'LEHRER'
                      ? 'Enseignant DSH/TestDaF'
                      : selectedProvider.role === 'BETREUER'
                      ? 'Betreuer & Logement Étudiant'
                      : 'Compagnon Visa Allemagne'
                  }
                  displayName={selectedProvider.name}
                  shortBio="Accompagnateur spécialisé pour la mobilité estudiantine germano-africaine."
                  fullDescription={`Spécialiste de l'accompagnement étudiant pour l'Allemagne.\n\nPrestations:\n- Inscription université & logement\n- Préparation visa et entretien\n- Entraînement linguistique.`}
                  ...
                  verified={selectedProvider.verified}
                  onContactClick={() => {
                    setToastMessage(`Redirection vers la messagerie avec ${selectedProvider.name}`);
                    setSelectedProvider(null);
                    setActiveTab('messages');
                  }}
                />
